...
import { View, Text,StyleSheet,Image,TouchableOpacity,TextInput,SafeAreaView,KeyboardAvoidingView,useWindowDimensions} from 'react-native'
...
const SignupOption3 = ({navigation}) => {
  ...
  const handleTextChange = (text) => {
    ...
    const formattedCode = cleanedText.split('').reverse().join('').padStart(6, '-');

    setCode(formattedCode);
  }

  return (
        <KeyboardAvoidingView behavior={`${padding}`} style={{flex:1,minHeight:windowWith, backgroundColor: '#2D665F',justifyContent: 'center',alignItems: 'center',}}>
            ...
            <Image
              ...
              <View style={{flex:0.4,justifyContent:'space-evenly',alignItems:'center',justifyContent:'center'}}>
                 <View style={{flex:0.6}}>
                    <View>
                        <Text style={styles.mid}>Code de vérification envoyé</Text>
                    </View>
                    <View style={{justifyContent:'center',alignItems:'center',marginVertical:20}}>
                        <Text style={{color:'#ccc'}}>Entrez les quatres chiffres qui</Text>
                        <Text style={{color:'#ccc'}}>ont été envoyés à l’adresse mail </Text>
                        <Text style={{marginVertical:7,fontSize:15,color:'#fff'}}>jeckodev@gmail.com</Text>
                    </View>
               </View>
               <View style={{flex:0.3}}>
               <TextInput
